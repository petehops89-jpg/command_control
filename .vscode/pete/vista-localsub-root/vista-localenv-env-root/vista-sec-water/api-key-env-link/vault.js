...
const SECRET_REGISTRY = {
  ...
  GOOGLE_CLOUD_PROJECT: {
    description: 'GCP Project ID (vists-498322)',
    required: false,
    defaultValue: 'vists-498322',
  },
  GOOGLE_CLOUD_LOCATION: {
    description: 'GCP default region/location',
    required: false,
    defaultValue: 'global',
  },
  GOOGLE_GENAI_USE_VERTEXAI: {
    description: 'Use Vertex AI SDK path (not Gemini API direct)',
    required: false,
    defaultValue: 'true',
  },
  GOOGLE_APPLICATION_CREDENTIALS: {
    description: 'Path to gcloud ADC JSON file',
    required: false,
    defaultValue: '%APPDATA%\\gcloud\\application_default_credentials.json',
  },
...
async function getSecret(name) {
  ...
  // 2. Windows Credential Manager (highest local priority — persists
  //    across reboots and isn't polluted by stale env vars)
  if (process.platform === 'win32' && entry.credentialTarget) {
    try {
      const pwsh = `(Get-StoredCredential -Target '${entry.credentialTarget}' -AsCredentialObject -ErrorAction Stop).Password`;
  ...
  // 3. Process environment (Docker / Node.js server)
  //    In Docker, these are injected at container start. On Windows,
  //    Credential Manager takes priority to avoid stale env pollution.
  const envVal = process.env[name];
  if (envVal && !process.platform === 'win32') return envVal;
  if (envVal && process.platform === 'win32' && !entry.credentialTarget) return envVal;

  // 4. Docker secret file (Docker Swarm / Compose secrets mount)
  try {
...
function storeSecret(name, value) {
  ...
  if (process.platform === 'win32' && entry.credentialTarget) {
    ...
    const pscmd = [
      ...
      `$pw = ConvertTo-SecureString $val -AsPlainText -Force`,
      `$cred = New-Object System.Management.Automation.PSCredential('${entry.credentialTarget}', $pw)`,
      `New-StoredCredential -Target '${entry.credentialTarget}' -Credentials $cred -Type Generic -Persist LocalMachine -ErrorAction Stop`,
    ].join('; ');
